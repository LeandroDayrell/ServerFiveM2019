...
Citizen.CreateThread(function()
	...
	while true do
		...
		if distance < 1.5 then
			...
			if golfplaying then
				DisplayHelpText("pressione ~g~~INPUT_CONTEXT~~s~ para terminar o golfe.")
			else
				DisplayHelpText("pressione ~g~~INPUT_CONTEXT~~s~ para começar o golfe (RS 100).")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					TriggerEvent("customNotification","Pressione E para balançar, A-D para girar, Y para trocar o clube.")
				end
			end
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Mova-se para a sua bola, pressione ~g~E~s~ a queda de bola se você estiver preso.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	if distance < 1.5 then
		TriggerEvent("customNotification","Voce conseguiu a bola com o alcance!")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		TriggerEvent("customNotification","Você pegou muitos tiros ..")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end

	Citizen.Trace("Ball parecia ter pousado: " .. GetCollisionNormalOfLastHitForEntity(mygolfball) )
end
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("pressione ~g~E~s~ para cair aqui.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("pressione ~g~E~s~derrubar - ~r~ muito longe da bola ou perto do buraco.")
		end
	end
end
