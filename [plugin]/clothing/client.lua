...
function customise(title)
    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarString({"Cabeca","mascarar","Cabelo","Bracos","calca","Para-quedas","sapatos","Colar & gravatas","Undershirt","Armadura corporal","Decalques","Camisas"}, componentScroller, function(cb)  componentScroller = cb end)) then
                subComponentScroller = GetPedDrawableVariation(GetPlayerPed(-1), componentScroller)
                textureScroller = GetPedTextureVariation(GetPlayerPed(-1), componentScroller)
                paletteScroller = GetPedPaletteVariation(GetPlayerPed(-1), componentScroller)
            end
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Componentes", subComponentScroller, GetNumberOfPedDrawableVariations(GetPlayerPed(-1), componentScroller), function(cb)  subComponentScroller = cb end)) then
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Palete de cores", paletteScroller, 2, function(cb)  paletteScroller = cb end)) then
                SetPedComponentVariation(GetPlayerPed(-1), componentScroller, subComponentScroller, textureScroller, paletteScroller)
            end
        end)
        Menu.addOption("clothing_customise", function()
        ...
        paletteScroller = GetPedPaletteVariation(GetPlayerPed(-1), componentScroller)
        Menu.addOption("clothing_customise", function()
            local precomponentTable = {"Cabeca","Nenhuma ideia","Cabelo","Camisas","calca","Nenhuma ideia","Nenhuma ideia","Nenhuma ideia","Colar & gravatas","Nenhuma ideia","Nenhuma ideia","Nenhuma ideia"}
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Texturas", textureScroller, GetNumberOfPedTextureVariations(GetPlayerPed(-1), componentScroller, subComponentScroller), function(cb)  textureScroller = cb end)) then
...
function accessories(title)
    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarString({"Chapeus/Capacetes","oculos","Brincos","Slot vazio","Slot vazio","Slot vazio","Pulso esquerdo","Pulso direito"}, componentScroller, function(cb)  componentScroller = cb end)) then
                subComponentScroller = GetPedPropIndex(GetPlayerPed(-1), componentScroller)
                textureScroller = GetPedPropTextureIndex(GetPlayerPed(-1), componentScroller)
            end
        end)
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarInt("Componentes", subComponentScroller, GetNumberOfPedPropDrawableVariations(GetPlayerPed(-1), componentScroller), function(cb)  subComponentScroller = cb end)) then
        ...
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarStringSelect({"Remova capacetes","Retire os oculos","Remover brincos","Remova o pulso esquerdo","Remova o pulso direito"}, removeScroller, function(cb)  removeScroller = cb end)) then
        ...
        local precomponentTable = {"Chapeus/Capacetes","oculos","Brincos","Slot vazio","Slot vazio","Slot vazio","Pulso esquerdo","Pulso direito"}
        local componentTable = {}
        for i = 0, 7 do
        ...
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarStringSelect({"Remova capacetes","Retire os aculos","Remover brincos","Remova o pulso esquerdo","Remova o pulso direito"}, removeScroller, function(cb)  removeScroller = cb end)) then
                if removeScroller ~= 3 and removeScroller ~= 4 then
                    ClearPedProp(GetPlayerPed(-1), tonumber(removeScroller))
                elseif removeScroller == 3 then
                    ClearPedProp(GetPlayerPed(-1), 6)
...
function overlays(title)
    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_overlays", function()
            if(Menu.ScrollBarString({"Manchas","Pelos faciais","Sobrancelhas","Envelhecimento","Maquiagem","Corar","Tez","Dano solar","Batom","Moles/Freckles","Cabelo de peito","Manchas do corpo","Adicionar manchas do corpo"}, componentScroller, function(cb)  componentScroller = cb end)) then
                subComponentScroller = GetPedHeadOverlayValue(GetPlayerPed(-1), componentScroller)
            end
        end)
        Menu.addOption("clothing_overlays", function()
            if(Menu.ScrollBarInt("Componentes", subComponentScroller, GetNumHeadOverlayValues(GetPlayerPed(-1), componentScroller), function(cb)  subComponentScroller = cb end)) then
                SetPedHeadOverlay(GetPlayerPed(-1), componentScroller, subComponentScroller, 1.0)
                opacityScroller = 1.0
            end
        end)
        Menu.addOption("clothing_overlays", function()
            if(Menu.ScrollBarInt("Opacidade", opacityScroller, 10, function(cb)  opacityScroller = cb end)) then
        ...
        Menu.addOption("clothing_overlays", function()
            if(Menu.ScrollBarInt("Cores", colourScroller, 63, function(cb)  colourScroller = cb end)) then
                local colourType = 0
