...
menu_pc[lang.police.pc.licenca.show.title()] = {ch_show_police_licenca,lang.police.pc.licenca.show.description()}
menu_pc[lang.police.pc.licenca.delete.title()] = {ch_delete_police_licenca, lang.police.pc.licenca.delete.description()}
...
-------------------------------------------------------
--------------------------------------------------------
---------- LICENÇA DE ARMA ATUALIZADA -------------------

    -- INSIRIR LICENÇA

    function vRP.insertPoliceLicenca(user_id, line)
      if user_id then
        local data = vRP.getUData(user_id, "vRP:police_licenca")
        local licenca = data..line.."<br />"
        vRP.setUData(user_id, "vRP:police_licenca", licenca)
      end
    end


    -- MOSTRAR LICENÇA
    local function ch_show_police_licenca(player,choice)
      local reg = vRP.prompt(player,lang.police.pc.searchreg.prompt(),"")
      local user_id = vRP.getUserByRegistration(reg)
      if user_id then
        local content = vRP.getUData(user_id, "vRP:police_licenca")
        vRPclient._setDiv(player,"police_pc",".div_police_pc{ background-color: rgba(0,0,0,0.75); color: white; font-weight: bold; width: 500px; padding: 10px; margin: auto; margin-top: 150px; }",content)
    ...
    -- DELETAR LICENÇA DO BANCO DE DADOS
    local function ch_delete_police_licenca(player,choice)
      local reg = vRP.prompt(player,lang.police.pc.searchreg.prompt(),"")
      local user_id = vRP.getUserByRegistration(reg)
      if user_id then
        vRP.setUData(user_id, "vRP:police_licenca", "")
        vRPclient._notify(player,lang.police.pc.licenca.delete.deleted())
      else
        vRPclient._notify(player,lang.common.not_found())
      end
    end

     --- BUSCAR LICENÇA DO BD
    local choice_licenca = {function(player, choice)
      ...
      if user_id then
        ...
        if nuser_id then
          ...
          -- build licenca menu
          local menu = {name=lang.police.menu.licenca.title(),css={top="75px",header_color="rgba(0,125,255,0.75)"}}

          local choose = function(player,choice) -- fine action
            local amount = cfg.licencas[choice]
            if amount ~= nil then
              if vRP.tryFullPayment(nuser_id, amount) then
                vRP.insertPoliceLicenca(nuser_id, lang.police.menu.licenca.licenca({choice,amount}))
                vRPclient._notify(player,lang.police.menu.licenca.licencad({choice,amount}))
                vRPclient._notify(nplayer,lang.police.menu.licenca.notify_licencad({choice,amount}))
          ...
          for k,v in pairs(cfg.licencas) do -- add fines in function of money available
            if v <= money then
              menu[k] = {choose,v}
            end
          end
    ...
    end, lang.police.menu.licenca.description()}
...
-------------------------------------------------------
-- POLICE ABRIR PORTA MALAS
local function ch_portamalas(player,choice)
  local nplayer = vRPclient.getNearestPlayer(player,10)
  local nuser_id = vRP.getUserId(nplayer)
  if nuser_id then
    vRPclient._notify(player,lang.police.menu.portamalas.abrir())
      local vtype,name = vRPclient.getNearestOwnedVehicle(nplayer,7)
  ...
  if user_id then
    ...
    if vRP.hasPermission(user_id,"police.menu") then
      ...
      choices[lang.police.title()] = {function(player,choice)
		...
		if vRP.hasPermission(user_id,"police.portamalas") then
		  menu[lang.police.menu.portamalas.title()] = ch_portamalas
		end
...
local function build_client_points(source)
  ...
  for k,v in pairs(cfg.pcs) do
    ...
    vRPclient._addMarker(source,x,y,z-1,1.1,1.1,1.7,0,125,255,125,150)
