    ...
    if v and user_id then -- for each player transforming
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        if money_ok and reagents_ok and inventory_ok then -- do transformation
          ...
          -- PRODUZIR DINHEIRO SUJO
          if recipe.out_dinheirosujo ~= nil then
          if recipe.out_dinheirosujo > 0 then vRP.giveDinheirosujo(user_id,recipe.out_dinheirosujo) end
          for l,w in pairs(recipe.products) do
            vRP.giveInventoryItem(user_id,l,w,true)
          end
          end
...
function vRP.setItemTransformer(name,itemtr)
  ...
  for action,recipe in pairs(tr.itemtr.recipes) do
    ...
    -------- FUNÇÃO QUE APARECE NA TELA DO GAME
    if recipe.out_dinheirosujo ~= nil then
        info = info.."<br /><span style=\"color: rgb(0,255,125)\">=></span>"
        if recipe.out_dinheirosujo > 0  then info = info.."<br />Dinheiro Sujo + "..recipe.out_dinheirosujo end
        for k,v in pairs(recipe.products) do
          local item = vRP.items[k]
          if item then
            info = info.."<br />"..v.." "..item.name
...
local function informer_placement_tick()
  ...
  for k,v in pairs(vRP.rusers) do
    ...
    vRPclient._setNamedMarker(player,"vRP:informer",x,y,z-1,1.1,1.1,1.7,0,255,125,125,150)
