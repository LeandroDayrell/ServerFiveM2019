  ...
  if ids ~= nil and #ids > 0 then
    ...
    if user_id then -- check user validity 
      ...
      if not vRP.isBanned(user_id) then
        deferrals.update("[vRP] Verificando Whitelist...")
        if not config.whitelist or vRP.isWhitelisted(user_id) then
          ...
          print("[vRP] "..name.." ("..vRP.getPlayerEndpoint(source)..") Faça a WhiteList no Discord: https://discord.gg/SNbSreZ pegue seu (user_id = "..user_id..")")
          deferrals.done("Faça a WhiteList no Discord: https://discord.gg/SNbSreZ pegue seu (user_id = "..user_id..").")
		  Citizen.Wait(1000)
        end
      else
...
AddEventHandler("vRPcli:playerSpawned", function()
  ...
  if user_id then
    ...
    -- show loading
    vRPclient.setProgressBar(player,{"vRP:loading", "center", "AGUARDE", 0,0,0, 0})
