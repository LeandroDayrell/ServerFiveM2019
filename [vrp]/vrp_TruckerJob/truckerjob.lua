...
local MissionData = {
    [0] = {1212.4463, 2667.4351, 38.79, 1000}, --x,y,z,money
    [1] = {-410.6452331543,1177.8696289063,325.64175415039, 1300}, --x,y,z,money
    [2] = {2574.5144, 328.5554, 108.45, 1800},
    [3] = {-2565.0894, 2345.8904, 33.06, 2000},
    [4] = {1706.7966, 4943.9897, 42.16, 2500},
    [5] = {196.5617, 6631.0967, 31.53, 3000},
    [6] = {-567.66717529297,5356.8525390625,70.21849822998, 3200},
    [7] = {-1038.5166015625,4921.43359375,206.30116271973, 3600}
}
local MISSION = {}
MISSION.start = false
...
function tick()
    ...
    if( GetDistanceBetweenCoords( playerCoords, TruckingCompany[0]["x"], TruckingCompany[0]["y"], TruckingCompany[0]["z"] ) < 10) then
            if(GUI.showStartText == false) then
                ...
                --key controlling
                if(IsControlPressed(1, Keys["SPACE"]) and GUI.showMenu == false) then
                    --clear()
                    GUI.showMenu = true
                    GUI.menu = 0
                end
                if(IsControlPressed(1, Keys["F1"]) and GUI.showMenu == true) then
                    GUI.showMenu = false
                end
            else
                GUI.showStartText = false
    ...
    elseif( MISSION.start == true ) then
        ...
        if( IsEntityAttached(MISSION.trailer) and text1 == false) then
            TriggerEvent("mt:missiontext", "Dirija até o marcado ~g~destination~w~.", 10000)
            text1 = true
        elseif( not IsEntityAttached(MISSION.trailer) and text2 == false ) then
            TriggerEvent("mt:missiontext", "Anexe o ~o~trailer~w~.", 15000)
            text2 = true
        end
        Wait(2000)
        ...
        if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and  not IsEntityAttached(MISSION.trailer)) then
            TriggerEvent("mt:missiontext", "Retorne o caminhão para as docas para iniciar o próximo trabalho ."..currentMission[4], 15000)
        ...
        elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) ) then
            TriggerEvent("mt:missiontext", "Chegou. Descarte seu ~o~trailer~w~ com ~r~H~w~ ", 15000)
        end
...
function GUI.drawStartText()
    TriggerEvent("mt:missiontext", "Você quer ser um camionista? pressione ~r~SPACE~w~ para iniciar.", 500)
...
function GUI.init()
    ...
    GUI.addButton(1, "Mission 1 [ 1.000$ ]", GUI.mission, 0.35, 0.25, 0.3, 0.05)
    GUI.addButton(1, "Mission 3 [ 1.300$ ]", GUI.mission, 0.35, 0.30, 0.3, 0.05)
    GUI.addButton(1, "Mission 4 [ 1.800$ ]", GUI.mission, 0.35, 0.35, 0.3, 0.05)
    GUI.addButton(1, "Mission 5 [ 2.000$ ]", GUI.mission, 0.35, 0.40, 0.3, 0.05)
    GUI.addButton(1, "Mission 6 [ 2.500$ ]", GUI.mission, 0.35, 0.45, 0.3, 0.05)
    GUI.addButton(1, "Mission 7 [ 3.000$ ]", GUI.mission, 0.35, 0.50, 0.3, 0.05)
    GUI.addButton(1, "Mission 8 [ 3.200$ ]", GUI.mission, 0.35, 0.55, 0.3, 0.05)
    GUI.addButton(1, "Mission 9 [ 3.600$ ]", GUI.mission, 0.35, 0.60, 0.3, 0.05)
