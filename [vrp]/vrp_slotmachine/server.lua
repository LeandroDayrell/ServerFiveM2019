...
AddEventHandler('vrp_slotmachine:server:1', function(amount,a,b,c)
	...
	amount = tonumber(amount)
		...
		TriggerClientEvent('chatMessage', source, "", {0, 0, 200}, "^1Você não tem dinheiro suficiente na sua carteira!") -- this is the message u get when u got no money
	end
end)
...
AddEventHandler('vrp_slotmachine:server:2', function(amount)
	...
	local user_id = vRP.getUserId({source})
	    ...
	    TriggerClientEvent('chatMessage', source, "", {0, 200, 60}, "Voce Ganhou:  " .. amount .. "$.") -- this is what u get if u win, u already get another one but this shows amount
	end)
