...
function vRPhk.canUserLockVehicle(plate, vehicleId, isPlayerInside)
	...
	local randomMsg = {	
		"Você encontrou as chaves no protetor solar",
     	"Você encontrou as chaves na caixa de luvas.",
    	"Você encontrou as chaves no assento do passageiro.",
	...
	for i=1, #(vehStorage) do
		if vehStorage[i].plate == plate then
			...
			if vehStorage[i].owner == playerIdentifier then
				...
				vRPclient.notifyPicture(player,"CHAR_LIFEINVADER", 3, "Sistema de bloqueio", "Chave", "Você não tem a chave deste veículo.")
				break
			end
		end
	end
	...
	if result == 0 and isPlayerInside then
		...
		vRPclient.notifyPicture(player,"CHAR_LIFEINVADER", 3, "Sistema de bloqueio", "Chave", randomMsg[randomNbr])
