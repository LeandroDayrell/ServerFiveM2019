	      ...
	      if skipper or caller then
		    local docs = HKserver.docsOnline()
		      if docs == 0 and skipper then
			    vRP.notify("~r~Não há ninguém para reviver você.\n~b~pressione ~g~E~b~ to respawn.")
			  elseif docs > 0 and caller then
			    vRP.notify("~g~Existem bombeiro on-line.\n~b~pressione ~g~E~b~ para chamar uma bombeiro.")
			  end
          end
	  else
	    called = called - 1
...
function vRPhk.lockVehicle(lockStatus, vehicle)
	if lockStatus == 1 or lockStatus == 0 then -- locked or unlocked
		...
		-- ## Notifications
			...
			vRP.notifyPicture("CHAR_LIFEINVADER", 3, "Sistema de bloqueio", "Chave", "Veículo trancado.")
		-- ## Notifications
	...
	elseif lockStatus == 2 then -- if it is locked
		...
		-- ## Notifications
			...
			vRP.notifyPicture("CHAR_LIFEINVADER", 3, "Sistema de bloqueio", "chave", "Veículo desbloqueado.")
		-- ## Notifications
